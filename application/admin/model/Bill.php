...
class Bill extends Model
{
    ...
    public function getBillImage($config=[])
    {
        $backImg = $config['backImg'];
        $new = $config['new_img'];
        $goods_img = $config['goods_img'];
        $codeImg = $config['qr_code'];
        try{
            if ($config['type'] ===1){
                // 添加二维码
                $this->addPic($backImg,$codeImg,110,110,415,300,$new);
                // 添加产品
                $this->addPic($new,$goods_img,150,150,15,20,$new);
                // 添加产品描述，对描述进行分行
                $theTitle = $this->cn_row_substr($config['title'],3,11);
                $this->addWord($theTitle[1],65,185,18,'black',$new);
                $this->addWord($theTitle[2],65,225,18,'black',$new);
                $this->addWord($theTitle[3],65,265,18,'black',$new);
                // 添加价格1
                $this->addWord('￥'.$config['price_market'],65,380,24,'red',$new);
                // 添加价格2
                //addWord('原价'.$config['price_member'],25,640,18,'black',$new);
                Log::write($new);
                return $new;
            }elseif ($config['type']==2){
                // 添加二维码
                $this->addPic($backImg,$codeImg,110,110,485,500,$new);
                // 添加产品
                $this->addPic($new,$goods_img,250,250,85,90,$new);
                // 添加产品描述，对描述进行分行
                $theTitle = $this->cn_row_substr($config['title'],3,11);
                $this->addWord($theTitle[1],105,405,20,'black',$new);
                $this->addWord($theTitle[2],105,445,20,'black',$new);
                $this->addWord($theTitle[3],105,485,20,'black',$new);
                // 添加价格1
                $this->addWord('￥'.$config['price_market'],105,580,24,'red',$new);
                // 添加价格2
                //addWord('原价'.$config['price_member'],25,640,18,'black',$new);
                Log::write($new);
                return $new;
    ...
    public function addWord($str,$posX,$poxY,$font,$color,$new)
    {
        ...
        imagettftext($s_original, $font, $tilt, $posX, $poxY, $ImgColor[$color], $fontfile, $str);
